    ...
    data: {
     // message: questions[0].questions,
      ...
      items: money, // money iterm list for each answer 
		...
		fiftyPressed: false //Flag sử dụng quyền trợ giúp 50:50
    },		
    ...
    computed: {
     reverseQs() {
      return _.reverse(this.questions);
		...
		//Trợ giúp 50:50
		fiftyHelp: function(){
			//Lấy index câu trả lời đúng
			var index = this.answers.indexOf(this.correct_answer);
			//Giữ lại câu đúng và loại bỏ 2 câu sai
            this.answers.splice(0, 4, this.answers[index], this.answers[(index +1) % 4]);            
            //Thiết lập Flag đã sử dụng quyền trợ giúp 50:50
			this.fiftyPressed = true;
			speechSynthesis.cancel();
			//Đọc lại 2 câu trả lời
